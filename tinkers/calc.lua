...
local function formula(atribute, input)
            ...
            for tool_combo = 1, max_materials do
                ...
                if #Tool_list < MAX_COUNT then
                    ...
                    for i = 2, #Tool_list do
                        if formula(atribute, Tool_list[i]) < formula(atribute, Tool_list[min_durability_index]) then
                            min_durability_index = i
                        end
                    end

                    -- Replace the tool with minimum durability if the new tool is better
                    if formula(atribute, newTool) > formula(atribute, Tool_list[min_durability_index]) then
...
Material_load();
best_parts(MAX_SIZE, param)
calculate(MAX_SIZE, param);
